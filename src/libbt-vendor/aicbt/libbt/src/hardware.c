...
extern uint8_t aicbt_fw_log_enable;
...
uint8_t hw_set_fw_log(HC_BT_HDR *p_buf)
{
    ...
    if (p_buf) {
        ...
        *p++ = aicbt_fw_log_enable;
        if(aicbt_fw_log_enable == 0x01){
...
uint8_t hw_set_assert_resume(HC_BT_HDR *p_buf){
    uint8_t     *p;
    uint8_t     ret = TRUE;
    ...
    if (p_buf == NULL) {
        p_buf = (HC_BT_HDR *) bt_vendor_cbacks->alloc(BT_HC_HDR_SIZE + \
                                                       HCI_CMD_PREAMBLE_SIZE + \
                                                       HCI_VSC_ASSERT_RESUME_SET_SIZE);
    }

    memset(p_buf, 0, BT_HC_HDR_SIZE + HCI_CMD_PREAMBLE_SIZE + HCI_VSC_ASSERT_RESUME_SET_SIZE);

    if (p_buf) {
        p_buf->event = MSG_STACK_TO_HC_HCI_CMD;
        p_buf->offset = 0;
        p_buf->layer_specific = 0;
        p_buf->len = HCI_CMD_PREAMBLE_SIZE + HCI_VSC_ASSERT_RESUME_SET_SIZE;
        p = (uint8_t *)(p_buf + 1);

        UINT16_TO_STREAM(p, HCI_VSC_ASSERT_RESUME_SET);
        *p++ = HCI_VSC_ASSERT_RESUME_SET_SIZE;
        ...
        *p++ = 0x00;
#endif
        hw_cfg_cb.state = HW_CFG_SET_ASSERT_RESUME;
        
        bt_vendor_cbacks->xmit_cb(HCI_VSC_ASSERT_RESUME_SET, p_buf, hw_config_cback);
...
uint8_t hw_get_fw_ver(HC_BT_HDR *p_buf){
    uint8_t     *p;
    uint8_t     ret = TRUE;
    ...
    if (p_buf == NULL) {
        p_buf = (HC_BT_HDR *) bt_vendor_cbacks->alloc(BT_HC_HDR_SIZE + \
                                                       HCI_CMD_PREAMBLE_SIZE + \
                                                       HCI_VSC_FW_VERSION_GET_SIZE);
    }

    memset(p_buf, 0, BT_HC_HDR_SIZE + HCI_CMD_PREAMBLE_SIZE + HCI_VSC_FW_VERSION_GET_SIZE);

    if (p_buf) {
        p_buf->event = MSG_STACK_TO_HC_HCI_CMD;
        p_buf->offset = 0;
        p_buf->layer_specific = 0;
        p_buf->len = HCI_CMD_PREAMBLE_SIZE + HCI_VSC_FW_VERSION_GET_SIZE;
        p = (uint8_t *)(p_buf + 1);

        UINT16_TO_STREAM(p, HCI_VSC_FW_VERSION_GET);
        *p++ = HCI_VSC_FW_VERSION_GET_SIZE;
        
        /*config id*/
        UINT16_TO_STREAM(p, AICBT_CONFIG_ID_FW_VER_GET);
        /*config len*/
        UINT16_TO_STREAM(p, AICBT_CONFIG_ID_FW_VER_GET_SIZE);

        *p++ = 0x01;

        hw_cfg_cb.state = HW_CFG_GET_FW_VERSION;
        
        bt_vendor_cbacks->xmit_cb(HCI_VSC_FW_VERSION_GET, p_buf, hw_config_cback);

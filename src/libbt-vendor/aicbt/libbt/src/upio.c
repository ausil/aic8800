...
static int init_rfkill()
{
    ...
    while ((de = readdir(d))) {
        if (strstr(de->d_name, "rfkill")) {
            ...
            if (sz >= 9 && memcmp(buf, "bluetooth", 9) == 0) {
                AICBTDBG(LOGINFO, "%s: rfkill path %s/%s", __func__, basepath, de->d_name);
...
fail:
    AICBTDBG(LOGERROR, "%s: No rfkill control node found", __func__);
    return -1;
#endif
}
...
void upio_set(uint8_t pio, uint8_t action, uint8_t polarity)
{
    ...
    AICBTDBG(LOGINFO, "%s : pio %d action %d, polarity %d", __FUNCTION__, pio, action, polarity);

    switch (pio) {
        case UPIO_LPM_MODE:
            if (upio_state[UPIO_LPM_MODE] == action) {
                AICBTDBG(LOGINFO, "LPM is %s already", lpm_mode[action]);
                return;
            }
            ...
            if (fd < 0) {
                AICBTDBG(LOGERROR, "upio_set : open(%s) for write failed: %s (%d)",
            ...
            if (write(fd, &buffer, 1) < 0) {
                AICBTDBG(LOGERROR, "upio_set : write(%s) failed: %s (%d)",
                        VENDOR_LPM_PROC_NODE, strerror(errno),errno);
        ...
        case UPIO_BT_WAKE:
            if (upio_state[UPIO_BT_WAKE] == action) {
                AICBTDBG(LOGINFO, "BT_WAKE is %s already", lpm_state[action]);
            ...
            AICBTDBG(LOGINFO, "%s: proc btwrite assertion, buffer: %c, timer_armed %d %d",
        ...
        case UPIO_HOST_WAKE:
            AICBTDBG(LOGINFO, "upio_set: UPIO_HOST_WAKE");
            break;
    }
}

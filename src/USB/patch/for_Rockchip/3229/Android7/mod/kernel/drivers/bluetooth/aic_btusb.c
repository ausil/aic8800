...
static void btusb_bulk_complete(struct urb *urb)
{
    ...
    if (urb->status == 0) {
        ...
        #else
        uint8_t *data = urb->transfer_buffer;
        if (hci_recv_fragment(hdev, data[0],
                        &data[1],
...
#if 0//AIDEN
		hdev->align_data = (u8*)kmalloc(sizeof(u8) * skb->len + align_param, GFP_ATOMIC);
			
		align = ((unsigned long)(hdev->align_data)) & (align_param - 1);
		memcpy(hdev->align_data + (align_param - align), skb->data, skb->len);
		usb_fill_bulk_urb(urb, data->udev, pipe,
						hdev->align_data + (align_param - align), skb->len, btusb_tx_complete, skb);
#else
		usb_fill_bulk_urb(urb, data->udev, pipe,
			skb->data, skb->len, btusb_tx_complete, skb);
#endif
...
static int __init btusb_init(void)
{
	...
	AICBT_INFO("RELEASE DATE: 2021_1027_1609 \r\n");
#if CONFIG_BLUEDROID
    err = btchr_init();
    if (err < 0) {

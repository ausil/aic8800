...
int rwnx_send_txpwr_ofst_req(struct rwnx_hw *rwnx_hw)
{
	...
	if(txpwr_ofst->enable){
		...
		printk("%s:Do not use txpwr_ofst\r\n", __func__);
                             ...
                             struct sm_connect_cfm *cfm)
{
	...
	printk("%s connect to %s auth_type:%d\r\n", __func__, sme->ssid, req->auth_type);
...
char ccode_channel[200];
int index_for_channel_list = 0;
module_param_string(ccode_channel, ccode_channel, 200, 0600);
...
//#ifdef CONFIG_RWNX_DBG
		if(chan->center_freq >= 2412 && chan->center_freq <= 2472){
			printk("scan channel:%d(%d) \r\n", (chan->center_freq - 2407)/5, chan->center_freq);

			sprintf(channel, "%d",((chan->center_freq - 2407)/5));
			memcpy(ccode_channel + index_for_channel_list, channel, strlen(channel));
			index_for_channel_list += strlen(channel);
			memcpy(ccode_channel + index_for_channel_list, " ", 1);
			index_for_channel_list += 1;
			
		}else if(chan->center_freq == 2484){
			printk("scan channel:14(%d) \r\n", chan->center_freq);
			
			sprintf(channel, "%d",14);
			memcpy(ccode_channel + index_for_channel_list, channel, strlen(channel));
			index_for_channel_list += strlen(channel);
			memcpy(ccode_channel + index_for_channel_list, " ", 1);
			index_for_channel_list += 1;
		}else{
			printk("scan channel:%d(%d) \r\n", ((chan->center_freq - 5175)/5) + 35, chan->center_freq);
			
			sprintf(channel, "%d",((chan->center_freq - 5175)/5) + 35);
			memcpy(ccode_channel + index_for_channel_list, channel, strlen(channel));
			index_for_channel_list += strlen(channel);
			memcpy(ccode_channel + index_for_channel_list, " ", 1);
			index_for_channel_list += 1;

...
/*    if (!capable(CAP_NET_ADMIN)) {
	...
	if(!strncasecmp(command, CMD_SET_COUNTRY, strlen(CMD_SET_COUNTRY))) {
		...
		if((ret = regulatory_set_wiphy_regd(vif->rwnx_hw->wiphy, getRegdomainFromRwnxDB(country)))){
                              ...
                              struct cfg80211_scan_request *request)
{
	...
	if (p2p_working && RWNX_VIF_TYPE(rwnx_vif) != NL80211_IFTYPE_P2P_CLIENT &&
		...
		printk("p2p is working, scan abort\n");
		return -EBUSY;
	}
#endif
    if (scanning) {
...
#if 0 //disable he for tp link
	if(sme->bssid[0] == 0x3c &&
		sme->bssid[1] == 0x06 &&
		sme->bssid[2] == 0xa7 &&
		sme->bssid[3] == 0x87 &&
		sme->channel->center_freq >= 2484){
		rwnx_hw->he_flag = false;
	}else{
		rwnx_hw->he_flag = true;
	}
	rwnx_send_me_config_req(rwnx_hw);
#endif
                                  ...
                                  struct cfg80211_ap_settings *settings)
{
    ...
    if (error) {
        ...
        netdev_info(dev, "AP started: ch=%d, bcmc_idx=%d channel=%d",
                    rwnx_vif->ch_index, rwnx_vif->ap.bcmc_index, ((settings->chandef).chan)->center_freq);
    }
                                ...
                                unsigned int duration, u64 *cookie)
{
	...
	if(duration < 100){
		printk("%s duration time change to 200ms \r\n", __func__);
		duration = 200;
	}
#endif
...
#else
	ret = rwnx_send_set_stack_start_req(rwnx_hw, 1, get_hardware_info(), feature.hwinfo, 0, &set_start_cfm);
#endif
...
//#endif
    ...
    if (mac_addr_efuse[0] | mac_addr_efuse[1] | mac_addr_efuse[2] | mac_addr_efuse[3])
    {
		...
		printk("no mac address in efuse!");
	}

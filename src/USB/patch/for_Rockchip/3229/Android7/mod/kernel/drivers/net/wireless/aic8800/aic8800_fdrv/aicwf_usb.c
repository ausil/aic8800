...
static void aicwf_usb_rx_complete(struct urb *urb)
{
	...
	if(!usb_dev->rwnx_hw){
		printk("usb_dev->rwnx_hw is not ready \r\n");
		return;
	}
...
#ifndef CONFIG_RX_TASKLET //AIDEN tasklet 
...
//	int i = 0;
    ...
    while((usb_buf = aicwf_usb_rx_buf_get(usb_dev)) != NULL) {
		...
		#if 0
		if(i > AICWF_USB_RX_URBS){//workaround
			printk("AICWF_USB_RX_URBS out of number\r\n");
			return;
		}
		i++;
		#endif//AIDEN
...
int usb_bustx_thread(void *data)
{
	...
	usbdev->tx_thread_working_flag = 1;
    while (1) {
/*
        ...
        */ 
        if (!wait_for_completion_interruptible(&bus->bustx_trgg)) {
            ...
            if(usbdev->bus_if->state == BUS_DOWN_ST){
				usb_err("usb bustx thread stop 1\n");
                break;
            }
			spin_lock_bh(&usbdev->tx_thread_working);
            if (usbdev->tx_post_count > 0){
                aicwf_usb_tx_process(usbdev);
            }
			spin_unlock_bh(&usbdev->tx_thread_working);
        }
    }
	
	usbdev->tx_thread_working_flag = 0;
    return 0;
}
...
int usb_busrx_thread(void *data)
{
    ...
    while (1) {
        ...
        if(kthread_should_stop()) {
            usb_err("usb busrx thread stop 1\n");
            break;
        }
...
void rwnx_data_dump(void* data, unsigned long len);
int align_param = 8;
...
static int aicwf_usb_bus_txdata(struct device *dev, struct sk_buff *skb)
{
    ...
    if (txhdr->sw_hdr->need_cfm) {
        need_cfm = true;
        buf = kmalloc(skb->len, GFP_ATOMIC/*GFP_KERNEL*/);
    ...
    if (usb_dev->tx_free_count < AICWF_USB_TX_LOW_WATER) {
		printk("AIDEN usb_dev->tx_free_count < AICWF_USB_TX_LOW_WATER:%d\r\n", 
			usb_dev->tx_free_count);
        usb_dev->tbusy = true;
...
static int aicwf_usb_init(struct aic_usb_dev *usb_dev)
{
	...
	spin_lock_init(&usb_dev->tx_thread_working);
...
void aicwf_usb_exit(void)
{
	...
	if(!g_rwnx_plat || !g_rwnx_plat->enabled){
		printk("g_rwnx_plat is not ready. waiting for 500ms\r\n");
		mdelay(500);
	}
#if 0
    if(g_rwnx_plat && g_rwnx_plat->enabled){

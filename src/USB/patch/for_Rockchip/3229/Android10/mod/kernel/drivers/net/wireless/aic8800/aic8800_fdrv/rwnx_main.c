...
int handle_private_cmd(struct net_device *net, char *command, u32 cmd_len)
{
    ...
    do {
		...
		if (argc == 5) {
			printk("btrf reset\n");
			dh_cmd_reset[0] = command_strtoul(argv[1], NULL, 16);
			dh_cmd_reset[1] = command_strtoul(argv[2], NULL, 16);
			dh_cmd_reset[2] = command_strtoul(argv[3], NULL, 16);
			dh_cmd_reset[3] = command_strtoul(argv[4], NULL, 16);
            printk("0x%x ", dh_cmd_reset[0]);
            printk("0x%x ", dh_cmd_reset[1]);
            printk("0x%x ", dh_cmd_reset[2]);
            printk("0x%x ", dh_cmd_reset[3]);
            printk("\n");
		...
		#ifdef AICWF_USB_SUPPORT
		rwnx_send_rftest_req(g_rwnx_plat->usbdev->rwnx_hw, BT_RESET, sizeof(dh_cmd_reset), (u8_l *)&dh_cmd_reset, NULL);
		#endif
	} else if(strcasecmp(argv[0], "BT_TXDH") == 0) {
		if (argc == 19) {
			printk("btrf txdh\n");
            for(i = 0; i < 18; i++) {
			dh_cmd_txdh[i] = command_strtoul(argv[i+1], NULL, 16);
            printk("0x%x ", dh_cmd_txdh[i]);
            }
            printk("\n");
		} else {
		    printk("wrong param\n");
		...
		#ifdef AICWF_USB_SUPPORT
		rwnx_send_rftest_req(g_rwnx_plat->usbdev->rwnx_hw, BT_TXDH, sizeof(dh_cmd_txdh), (u8_l *)&dh_cmd_txdh, NULL);
		#endif
	} else if(strcasecmp(argv[0], "BT_RXDH") == 0) {
		if(argc == 18) {
			printk("btrf rxdh\n");
            for(i = 0; i < 17; i++) {
			dh_cmd_rxdh[i] = command_strtoul(argv[i+1], NULL, 16);
            printk("0x%x ", dh_cmd_rxdh[i]);
            }
		} else {
			printk("wrong param\n");
			break;
		...
		#ifdef AICWF_USB_SUPPORT
		rwnx_send_rftest_req(g_rwnx_plat->usbdev->rwnx_hw, BT_RXDH, sizeof(dh_cmd_rxdh), (u8_l *)&dh_cmd_rxdh, NULL);
		#endif
	} else if(strcasecmp(argv[0], "BT_STOP") == 0) {
		if(argc == 6) {
            for(i = 0; i < 5; i++) {
			dh_cmd_stop[i] = command_strtoul(argv[i+1], NULL, 16);
            printk("0x%x ", dh_cmd_stop[i]);
            }
		} 
#endif
        else {
		...
		#ifdef AICWF_USB_SUPPORT
		rwnx_send_rftest_req(g_rwnx_plat->usbdev->rwnx_hw, BT_STOP, sizeof(dh_cmd_stop), (u8_l *)&dh_cmd_stop, NULL);
		#endif
	}
	  else {
...
static void patch_config(struct rwnx_hw *rwnx_hw)
{
    ...
    if((ret = rwnx_send_dbg_mem_write_req(rwnx_hw, 0x1e4d78, patch_addr))) {
        printk("%x write fail\n", 0x1e4d78);
    }
    ...
    #else
    if((ret = rwnx_send_dbg_mem_write_req(rwnx_hw, 0x1e4d7c, patch_num))) {
        printk("%x write fail\n", 0x1e4d7c);
    }
    #endif

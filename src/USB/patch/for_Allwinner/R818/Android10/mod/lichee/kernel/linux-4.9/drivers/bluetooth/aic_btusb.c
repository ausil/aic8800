...
//AIDEN
    AICBT_DBG("%s: count:%d data:%p &hdev->flags:%p", __func__, urb->actual_length, data, &hdev->flags);

    if (!test_bit(HCI_RUNNING, &hdev->flags)) {
        printk("%s HCI_RUNNING\n", __func__);
        return;
    }
    AICBT_DBG("%s: IN", __func__);
    if (urb->status == 0) {
        ...
        if(urb->actual_length > 1){
            AICBT_DBG("%s: data_[0]:%p data_[0]:%d", __func__, &(data_[0]), data_[0]);

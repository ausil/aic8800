...
void aicwf_patch_config_8800dc(struct rwnx_hw *rwnx_hw)
{
    ...
    if (testmode == 0) {
       ...
       AICWFDBG(LOGINFO, "wifisetting_cfg_addr=%x, ldpc_cfg_addr=%x, agc_cfg_addr=%x, txgain_cfg_addr=%x,jump_tbl_addr=%x\n", wifisetting_cfg_addr, ldpc_cfg_addr, agc_cfg_addr, txgain_cfg_addr,jump_tbl_addr);
        ...
        for (cnt = 0; cnt < jump_tbl_size/4; cnt+=1) {
            ...
            if ((ret = rwnx_send_dbg_mem_write_req(rwnx_hw, jump_tbl[cnt][0]*4+jump_tbl_addr, jump_tbl[cnt][1]))) {
                AICWFDBG(LOGERROR, "%x write fail\n", jump_tbl_addr+8*cnt);
            }
        }
        for (cnt = 0; cnt < patch_tbl_func_num; cnt++) {
            if ((ret = rwnx_send_dbg_mem_write_req(rwnx_hw, patch_tbl_func[cnt][0], patch_tbl_func[cnt][1]))) {
                AICWFDBG(LOGERROR, "patch_tbl_func %x write fail\n", patch_tbl_func[cnt][0]);
            }
        }
        #endif
    } else {

...
module_param(sdio_f1, int, 0660);
module_param(sdio_f8, int, 0660);
module_param(sdio_f0, int, 0660);
module_param(sdio_f3, int, 0660);
...
int aicwf_sdiov3_func_init(struct aic_sdio_dev *sdiodev)
{
    ...
    sdio_f0_writeb(sdiodev->func, sdio_f0, 0xF0, &ret);
    if (ret) {
        sdio_err("set iopad ctrl fail %d\n", ret);
        sdio_release_host(sdiodev->func);
        return ret;
    }
    sdio_f0_writeb(sdiodev->func, sdio_f8, 0xF8, &ret);
    if (ret) {
        sdio_err("set iopad delay2 fail %d\n", ret);
        sdio_release_host(sdiodev->func);
        return ret;
    }
    sdio_f0_writeb(sdiodev->func, sdio_f1, 0xF1, &ret);
    if (ret) {
        sdio_err("set iopad delay1 fail %d\n", ret);
        sdio_release_host(sdiodev->func);
        return ret;
    }
    sdio_f0_writeb(sdiodev->func, sdio_f3, 0xF3, &ret);
    if (ret) {

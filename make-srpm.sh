...
# Copy SDIO firmware (all chip variants into a flat sdio/ dir)
mkdir -p "$WORKDIR/$TARDIR/firmware/sdio"
for fwdir in \
    "$SCRIPT_DIR/src/SDIO/driver_fw/fw/aic8800" \
    "$SCRIPT_DIR/src/SDIO/driver_fw/fw/aic8800D80" \
    "$SCRIPT_DIR/src/SDIO/driver_fw/fw/aic8800D80X2" \
    "$SCRIPT_DIR/src/SDIO/driver_fw/fw/aic8800D80N" \
    "$SCRIPT_DIR/src/SDIO/driver_fw/fw/aic8800DC"; do
    [ -d "$fwdir" ] && cp -a "$fwdir/"* "$WORKDIR/$TARDIR/firmware/sdio/" || :
done

# Copy USB firmware (multiple chip variants, flat into usb/)
...
for fwdir in \
    ...
    "$SCRIPT_DIR/src/USB/driver_fw/fw/aic8800D80N" \
    "$SCRIPT_DIR/src/USB/driver_fw/fw/aic8800DC" \
    "$SCRIPT_DIR/src/USB/driver_fw/fw/aic8800DLN"; do
    [ -d "$fwdir" ] && cp -a "$fwdir/"* "$WORKDIR/$TARDIR/firmware/usb/" || :
done

# Copy PCIe firmware (D80 and D80X2 variants, flat into pcie/)
...
for fwdir in \
    ...
    [ -d "$fwdir" ] && cp -a "$fwdir/"* "$WORKDIR/$TARDIR/firmware/pcie/" || :
done
